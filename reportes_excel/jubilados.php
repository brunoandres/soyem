...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'REPORTE DE AFILIADOS JUBILADOS ACTIVOS Y COSEGURO')
            ->setCellValue('A2', 'NOMBRE')
            ->setCellValue('B2', 'LEGAJO')
            ->setCellValue('C2', 'DOCUMENTO')
			->setCellValue('D2', 'COSEGURO')
			->setCellValue('E2', 'IPROSS');
...
/*Extraer datos de MYSQL*/
	...
	$sql="select * from afiliado where activo = 'si' and coseguro ='si' and jubilado = 'si'";
	$query=mysqli_query($con,$sql);
	$cel=3;//Numero de fila donde empezara a crear  el reporte
	while ($row=mysqli_fetch_array($query)){
		$countryCode=$row['nombre'];
		$countryName=$row['legajo'];
		$currencyCode=$row['documento'];
		$capital=$row['coseguro'];
		$continentName=$row['ipross'];
...
$objPHPExcel->getActiveSheet()->setTitle('Reporte de Afiliados Jubilados');
...
header('Content-Disposition: attachment;filename="jubilados.xls"');
